...
const TOOLS = [
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        entity_id: { type: 'string', description: 'ID campagne ou ad set' },
        entity_type: { type: 'string', enum: ['campaign', 'adset', 'ad'], description: 'Type d\'entité' },
        start_date: { type: 'string', description: 'Date début YYYY-MM-DD. Défaut: 30 jours avant hier' },
        end_date: { type: 'string', description: 'Date fin YYYY-MM-DD. Défaut: hier' }
      },
      required: ['account_id', 'entity_id', 'entity_type']
    }
  },
  {
    name: 'search_targeting',
    description: 'Recherche d\'options de ciblage : intérêts, géolocalisation, comportements, données démographiques.',
    inputSchema: {
      type: 'object',
      properties: {
        account_id: { type: 'string', description: 'ID du compte pub' },
        query: { type: 'string', description: 'Mot-clé à rechercher (ex: restaurant, Paris, gastronomie)' },
        search_type: { type: 'string', enum: ['interests', 'geolocation', 'behaviors', 'locale'], description: 'Type de ciblage' }
      },
      required: ['account_id', 'query', 'search_type']
    }
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        account_id: { type: 'string', description: 'ID du compte pub' },
        targeting_spec: { type: 'object', description: 'Spec de ciblage Meta (geo_locations, interests, age_min, age_max, etc.)' },
        optimization_goal: { type: 'string', description: 'Objectif d\'optimisation. Ex: REACH, OFFSITE_CONVERSIONS' }
      },
      required: ['account_id', 'targeting_spec']
    }
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        account_id: { type: 'string', description: 'ID du compte pub' },
        subtype: { type: 'string', enum: ['CUSTOM', 'LOOKALIKE', 'WEBSITE', 'ENGAGEMENT', 'ALL'], description: 'Type. Défaut: ALL' }
      },
      required: ['account_id']
    }
  },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        creative_id: { type: 'string', description: 'ID du créatif' },
        ad_format: { type: 'string', enum: ['DESKTOP_FEED_STANDARD', 'MOBILE_FEED_STANDARD', 'INSTAGRAM_STANDARD', 'INSTAGRAM_STORY', 'AUDIENCE_NETWORK_OUTSTREAM_VIDEO'], description: 'Format d\'aperçu. Défaut: MOBILE_FEED_STANDARD' }
      },
      required: ['creative_id']
    }
  },
...

  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        entity_type: { type: 'string', enum: ['campaign', 'adset', 'ad'] },
        action: { type: 'string', enum: ['pause', 'resume'], description: 'Action à effectuer' }
      },
      required: ['entity_id', 'entity_type', 'action']
    }
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        entity_id: { type: 'string', description: 'ID de l\'entité' },
        entity_type: { type: 'string', enum: ['campaign', 'adset'] },
        daily_budget: { type: 'number', description: 'Budget journalier en centimes (1000 = 10€)' },
        lifetime_budget: { type: 'number', description: 'Budget total en centimes (optionnel)' }
      },
      required: ['entity_id', 'entity_type']
    }
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        campaign_id: { type: 'string', description: 'ID de la campagne à dupliquer' },
        new_name: { type: 'string', description: 'Nom de la nouvelle campagne (optionnel)' },
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        adset_id: { type: 'string', description: 'ID de l\'ad set à dupliquer' },
        new_name: { type: 'string', description: 'Nom du nouvel ad set (optionnel)' },
        target_campaign_id: { type: 'string', description: 'Campagne cible (optionnel, défaut: même campagne)' },
        status: { type: 'string', enum: ['PAUSED', 'ACTIVE'], description: 'Statut initial. Défaut: PAUSED' }
      },
      required: ['adset_id']
    }
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        ad_id: { type: 'string', description: 'ID de la publicité à dupliquer' },
        new_name: { type: 'string', description: 'Nom de la nouvelle publicité (optionnel)' },
        target_adset_id: { type: 'string', description: 'Ad set cible (optionnel)' },
        status: { type: 'string', enum: ['PAUSED', 'ACTIVE'], description: 'Statut initial. Défaut: PAUSED' }
      },
      required: ['ad_id']
    }
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        retention_days: { type: 'number', description: 'Durée de rétention en jours (1-180)' },
        event_name: { type: 'string', description: 'Événement pixel (Purchase, Lead, ViewContent, etc.). Omis = tous visiteurs' },
        pixel_id: { type: 'string', description: 'ID du pixel (optionnel, auto-détecté)' },
        description: { type: 'string', description: 'Description de l\'audience (optionnel)' }
      },
      required: ['account_id', 'name', 'retention_days']
    }
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        account_id: { type: 'string', description: 'ID du compte pub' },
        name: { type: 'string', description: 'Nom de l\'audience' },
        origin_audience_id: { type: 'string', description: 'ID de l\'audience source (min 100 personnes)' },
        country: { type: 'string', description: 'Code pays ISO (ex: FR, US, GB)' },
        ratio: { type: 'number', description: 'Pourcentage de similarité (0.01 = 1%, max 0.20). Défaut: 0.01' }
      },
      required: ['account_id', 'name', 'origin_audience_id', 'country']
    }
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        adset_id: { type: 'string', description: 'ID de l\'ad set' },
        targeting: { type: 'object', description: 'Nouveau spec de ciblage Meta complet (geo_locations, interests, age_min, age_max, etc.)' }
      },
      required: ['adset_id', 'targeting']
    }
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        objective: { type: 'string', enum: ['OUTCOME_AWARENESS', 'OUTCOME_TRAFFIC', 'OUTCOME_ENGAGEMENT', 'OUTCOME_LEADS', 'OUTCOME_APP_PROMOTION', 'OUTCOME_SALES'], description: 'Objectif de la campagne' },
        status: { type: 'string', enum: ['PAUSED', 'ACTIVE'], description: 'Statut initial. Défaut: PAUSED' },
        daily_budget: { type: 'number', description: 'Budget journalier en centimes (optionnel)' },
        lifetime_budget: { type: 'number', description: 'Budget total en centimes (optionnel)' },
        special_ad_categories: { type: 'array', items: { type: 'string' }, description: 'Catégories spéciales (HOUSING, EMPLOYMENT, CREDIT). Laisser vide si aucune.' }
      },
      required: ['account_id', 'name', 'objective']
    }
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        account_id: { type: 'string', description: 'ID du compte pub' },
        campaign_id: { type: 'string', description: 'ID de la campagne parente' },
        name: { type: 'string', description: 'Nom de l\'ad set' },
        daily_budget: { type: 'number', description: 'Budget journalier en centimes' },
        targeting: { type: 'object', description: 'Spec de ciblage (geo_locations requis minimum)' },
        optimization_goal: { type: 'string', enum: ['OFFSITE_CONVERSIONS', 'LEAD_GENERATION', 'LINK_CLICKS', 'REACH', 'IMPRESSIONS', 'LANDING_PAGE_VIEWS'], description: 'Objectif d\'optimisation' },
        billing_event: { type: 'string', enum: ['IMPRESSIONS', 'LINK_CLICKS', 'THRUPLAY'], description: 'Événement de facturation. Défaut: IMPRESSIONS' },
        bid_strategy: { type: 'string', enum: ['LOWEST_COST_WITHOUT_CAP', 'LOWEST_COST_WITH_BID_CAP', 'COST_CAP'], description: 'Stratégie d\'enchère. Défaut: LOWEST_COST_WITHOUT_CAP' },
        status: { type: 'string', enum: ['PAUSED', 'ACTIVE'], description: 'Statut initial. Défaut: PAUSED' },
        start_time: { type: 'string', description: 'Date/heure de début ISO 8601 (optionnel)' },
        end_time: { type: 'string', description: 'Date/heure de fin ISO 8601 (optionnel)' }
      },
      required: ['account_id', 'campaign_id', 'name', 'daily_budget', 'targeting', 'optimization_goal']
    }
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        message: { type: 'string', description: 'Texte principal de la publicité' },
        link: { type: 'string', description: 'URL de destination' },
        image_hash: { type: 'string', description: 'Hash de l\'image (depuis upload_ad_image)' },
        video_id: { type: 'string', description: 'ID de la vidéo (depuis upload_ad_video, optionnel)' },
        headline: { type: 'string', description: 'Titre de la publicité' },
        description: { type: 'string', description: 'Description (optionnel)' },
        call_to_action_type: { type: 'string', enum: ['BOOK_TRAVEL', 'CONTACT_US', 'LEARN_MORE', 'SHOP_NOW', 'SIGN_UP', 'SUBSCRIBE', 'GET_OFFER', 'GET_QUOTE', 'BOOK_NOW', 'APPLY_NOW'], description: 'Bouton call-to-action' }
      },
      required: ['account_id', 'name', 'page_id', 'message', 'link']
    }
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        creative_id: { type: 'string', description: 'ID du créatif (depuis create_ad_creative)' },
        name: { type: 'string', description: 'Nom de la publicité' },
        status: { type: 'string', enum: ['PAUSED', 'ACTIVE'], description: 'Statut initial. Défaut: PAUSED' }
      },
      required: ['account_id', 'adset_id', 'creative_id', 'name']
    }
  ...
  {
    name: 'upload_ad_image',
    description: 'Upload une image publicitaire depuis une URL externe vers un compte Meta.',
    inputSchema: {
      type: 'object',
      properties: {
        account_id: { type: 'string', description: 'ID du compte pub' },
        image_url: { type: 'string', description: 'URL publique de l\'image (JPG, PNG, GIF)' },
        name: { type: 'string', description: 'Nom de l\'image (optionnel)' }
      },
      required: ['account_id', 'image_url']
    }
  },
  {
    name: 'upload_ad_video',
    description: 'Upload une vidéo publicitaire depuis une URL externe vers un compte Meta.',
    inputSchema: {
      type: 'object',
      properties: {
        account_id: { type: 'string', description: 'ID du compte pub' },
        video_url: { type: 'string', description: 'URL publique de la vidéo (MP4, MOV)' },
        title: { type: 'string', description: 'Titre de la vidéo (optionnel)' },
...
async function runTool(name, args) {
  switch (name) {
    ...
    case 'list_ad_accounts': {
      const data = await meta('/me/adaccounts', 'GET', null, {
        fields: 'id,name,account_status,currency,timezone_name,spend_cap,amount_spent',
        limit: args.limit || 50
      });
      return data.data || [];
    ...
    case 'get_performance': {
      ...
      return (await meta(path, 'GET', null, {
        fields: 'campaign_id,campaign_name,adset_id,adset_name,ad_id,ad_name,spend,impressions,clicks,ctr,cpm,cpc,cpl,cpp,reach,frequency,actions,action_values,cost_per_action_type,website_purchase_roas',
    ...
    case 'search_targeting': {
      ...
      const params = { type: typeMap[args.search_type] || 'adTargetingCategory', q: args.query, limit: 30 };
      if (classMap[args.search_type]) params.class = classMap[args.search_type];
      const data = await meta('/search', 'GET', null, params);
      return data.data || [];
    }

    case 'estimate_audience_size': {
      const data = await meta(`/act_${args.account_id}/reachestimate`, 'GET', null, {
        targeting_spec: JSON.stringify(args.targeting_spec),
        optimization_goal: args.optimization_goal || 'REACH'
      });
      return data;
    ...
    case 'list_ad_creatives': {
      return (await meta(`/act_${args.account_id}/adcreatives`, 'GET', null, {
        fields: 'id,name,title,body,image_url,thumbnail_url,object_story_spec,effective_object_story_id',
        limit: args.limit || 50
      })).data || [];
    }
    ...
    case 'preview_creative': {
      const format = args.ad_format || 'MOBILE_FEED_STANDARD';
      return (await meta(`/${args.creative_id}/previews`, 'GET', null, { ad_format: format })).data || [];
    }

    case 'change_entity_status': {
      const status = args.action === 'pause' ? 'PAUSED' : 'ACTIVE';
      const r = await meta(`/${args.entity_id}`, 'POST', { status });
      return { success: r.success, entity_id: args.entity_id, entity_type: args.entity_type, new_status: status };
    }
    ...
    case 'duplicate_adset': {
      const body = { status: args.status || 'PAUSED', deep_copy: true };
      if (args.new_name) body.rename_options = { rename_suffix: '' };
      if (args.target_campaign_id) body.campaign_id = args.target_campaign_id;
      const r = await meta(`/${args.adset_id}/copies`, 'POST', body);
      return { success: true, new_adset_id: r.copied_adset_id, status: args.status || 'PAUSED' };
    }
    ...
    case 'create_ad_creative': {
      const story_spec = {
        ...
        link_data: {
          ...
          name: args.headline || '',
          description: args.description || '',
          call_to_action: args.call_to_action_type ? { type: args.call_to_action_type, value: { link: args.link } } : undefined
        }
      };
      if (args.image_hash) story_spec.link_data.image_hash = args.image_hash;
      if (args.video_id) {
        story_spec.video_data = { video_id: args.video_id, message: args.message, title: args.headline || '' };
        delete story_spec.link_data;
      }
      const r = await meta(`/act_${args.account_id}/adcreatives`, 'POST', {
        name: args.name, object_story_spec: story_spec
      });
    ...
    case 'upload_ad_image': {
      ...
      return { success: true, hash: first?.hash, url: first?.url, name: args.name };
    }
...
function createMCPServer() {
  const server = new Server(
    { name: 'dose-meta-mcp', version: '2.0.0' },
    { capabilities: { tools: {} } }
  );
  server.setRequestHandler(ListToolsRequestSchema, async () => ({ tools: TOOLS }));
  server.setRequestHandler(CallToolRequestSchema, async (req) => {
...
app.get('/health', (req, res) => {
  res.json({ status: 'ok', server: 'dose-meta-mcp', version: '2.0.0', tools: TOOLS.length, meta_token: !!META_TOKEN });
});
...
const PORT = process.env.PORT || 3000;
app.listen(PORT, () => {
  console.log(`Dose Meta MCP v2.0 — ${TOOLS.length} outils — port ${PORT}`);
  console.log(`Token: ${META_TOKEN ? 'OK' : 'MANQUANT'}`);
});
